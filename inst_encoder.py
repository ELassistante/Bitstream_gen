...
muxA_list     = ['SRAM', 'RCL', 'RCR', 'RCT', 'RCB',  'R0', 'R1', 'R2', 'R3', 'ZEROS']
muxB_list     = ['PREV', 'RCL', 'RCR', 'RCT', 'RCB', 'IMM', 'R0', 'R1', 'R2',    'R3', 'ZEROS']

# SDIV, UADD, USUB, UMUL, UDIV, FXP_DIV, LWIPI, SWIPI 
# ALL ABOVE OPS ARE NOT IMPLEMENTED YET BUT THE OPCODE IS RESERVED IF NEEDED
...
ALU_op_list   = ['EXIT', 
                    ...
                    'BEQ', 'BNE', 'BLT', 'BGE',
                    'LWD', 'LWI', 'LWIPI', 'SWD', 'SWI', 'SWIPI', 
                    'NOP']

# BSFA --> INA if sign flag, else INB

reg_dest_list  = ['R0', 'R1', 'R2', 'R3']
reg_we_list    = ['0', '1']
muxF_list      = ['PREV', 'RCL', 'RCR', 'RCT', 'RCB']
...
rcs_imem_file = './cgra_imem.bit'
ker_kmem_file = './cgra_kmem.bit'

rcs_logger = logfunc.log2file(rcs_imem_file, CGRA_IMEM_WIDTH + 4) # + 0b<>, 
ker_logger = logfunc.log2file(ker_kmem_file, CGRA_KMEM_WIDTH)

rcs_instructions = [[rcs_nop_instr for _ in range(CGRA_IMEM_N_LINE)] for _ in range (RCS_NUM_CREG)]
...
# 1. instructions_dbl_max.py
# 2. instructions_dbl_min.py
...
for i in range(0,RCS_NUM_CREG):
    for instruction in rcs_instructions[i]:
        ...
        rcs_logger.log_line('0b'+instr_bits+', ')
        # print(instr_bits)
